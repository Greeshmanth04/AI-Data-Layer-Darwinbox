                 ...
                 <div key={u._id} className="flex justify-between items-center p-3 hover:bg-slate-50">
                    ...
                    <button 
                      ...
                      className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors ${
                        isMember ? 'bg-indigo-50 border-indigo-200 text-indigo-700 hover:bg-indigo-100' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
               ...
               return (
                 <div key={g._id} className="flex justify-between items-center p-3 hover:bg-slate-50">
                    ...
                    <button 
                      ...
                      className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors ${
                        isAssigned ? 'bg-indigo-50 border-indigo-200 text-indigo-700 hover:bg-indigo-100' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-100'
                      }`}
                    >
                      ...
                      <div key={coll.name} className={`bg-white border rounded-xl shadow-sm transition-colors ${hasAccess ? 'border-indigo-200' : 'border-slate-200'}`}>
                         <div className="flex items-center justify-between p-5">
                            <div className="flex flex-col">
                               ...
                               <p className="text-xs text-slate-500 font-medium">Fields: {coll.fields.length}</p>
                            </div>
                               ...
                               <div className="grid grid-cols-2 gap-6">
                                  <div className="bg-white p-4 rounded-lg border border-slate-200">
                                     ...
                                     <div className="border border-slate-200 bg-slate-50 rounded-lg max-h-36 overflow-y-auto p-1.5 scrollbar-thin scrollbar-thumb-slate-300">
                                       {coll.fields.map((field: string) => {
                                          const isAllowed = existingPerm.allowedFields?.length > 0 
                                              ? existingPerm.allowedFields.includes(field) 
                                              : true; // implicitly allowed when empty
                                              
                                          return (
                                            <label key={field} className="flex items-center gap-2.5 p-1.5 hover:bg-white rounded border border-transparent hover:border-slate-200 cursor-pointer transition-colors group">
                                               <input 
                                                 type="checkbox" 
                                                 ...
                                                 onChange={(e) => {
                                                    ...
                                                    if (e.target.checked) {
                                                        ...
                                                        newFields = newFields.filter((f:string) => f !== field);
                                                    }
                                                    
                                                    // Clean up: if everything is checked, reset to empty array for cleaner payload
                                                    if (newFields.length === coll.fields.length) newFields = [];
                                               ...
                                               />
                                               <span className={`text-[13px] font-mono transition-colors ${isAllowed ? 'text-indigo-700 font-bold' : 'text-slate-600 group-hover:text-slate-900'}`}>{field}</span>
                                            </label>
                                          );
                                       })}
